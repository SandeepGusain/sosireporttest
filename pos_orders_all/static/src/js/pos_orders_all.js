...
// Load Models here...
    ...
    models.load_models({
        model: 'pos.order',
        fields: ['name', 'id', 'date_order', 'partner_id', 'pos_reference', 'lines', 'amount_total', 'session_id', 'state', 'company_id','coupon_id','barcode'],
        domain: function(self){ return [['state', 'not in', ['draft', 'cancel']]]; }, //['session_id', '=', self.pos_session.name],
    ...
    var PosBarcodePopupWidget = popups.extend({
        ...
        renderElement: function() {
            ...
            this.$('#apply_barcode_return_order').click(function() {
                    ...
                    var selectedOrder = null;
		            for(var i = 0, len = Math.min(orders.length,1000); i < len; i++) {
		                console.log('11111111111111 vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv',orders[i], orders[i].barcode, entered_barcode)
		                if (orders[i] && orders[i].barcode == entered_barcode) {
